...
class EntryBytes(Entry):
    ...
    def Declaration(self):
        return [f'uint8_t {self._name}_data[{self._length}];']
...
class EntryInt(Entry):
    ...
    def Declaration(self):
        return [f'uint32_t {self._name}_data;']
...
class EntryVarBytes(Entry):
    ...
    def GetDeclaration(self, funcname):
        return [
            f'int {funcname}(struct {self._struct.Name()} *, {self._ctype} *, uint32_t *);'
        ]
        
    def AssignDeclaration(self, funcname):
        return [
            f'int {funcname}(struct {self._struct.Name()} *, const {self._ctype}, uint32_t);'
        ]
    ...
    def CodeAssign(self):
        ...
        return [
            'int',
            f'{self._struct.Name()}_{name}_assign(struct {self._struct.Name()} *msg, const {self._ctype} value, uint32_t len)',
            '{',
            f'  if (msg->{name}_data != NULL)',
    ...
    def CodeGet(self):
        ...
        return [
            'int',
            f'{self._struct.Name()}_{name}_get(struct {self._struct.Name()} *msg, {self._ctype} *value, uint32_t *plen)',
            '{',
            f'  if (msg->{name}_set != 1)',
            '    return (-1);',
            f'  *value = msg->{name}_data;',
    ...
    def Declaration(self):
        return [f'uint8_t *{self._name}_data;', f'uint32_t {self._name}_length;']
