...
def get_option_value_set(cp, option):
    return {
        cp.get(s, option)
        for s in filter(lambda s: cp.has_option(s, option), cp.sections())
    }

def get_node_dicts(cp, portbase):
    node_sections = filter(
        lambda s: cp.has_option(s, 'NodeId') and 'API' not in s, cp.sections()
    )
    ndicts = []
    for ns in node_sections:
        ...
        if t == 'MYSQLD':
            ...
            portbase += 1
        if cp.has_section(f'{t} DEFAULT'):
            nalist += cp.items(f'{t} DEFAULT')
...
def get_actual_section(s):
    return s if 'DEFAULT' in s else '_'.join(s.split('_')[:-1])

def get_proct1(s):
    s = s.rstrip(' DEFAULT') if 'DEFAULT' in s else '_'.join(s.split('_')[:-1])
    return s.lower()

def get_pid1(cp, s):
    return cp.get(s, 'NodeId') if cp.has_option(s,'NodeId') else None
    ...
    return [
        {
            ...
            'configfilepath': None,
            'ndbconnecturl': get_ndbconnecturl(cp),
        }
        for s in filter(
            lambda s: cp.has_option(s, 'NodeId') and 'API' not in s,
            cp.sections(),
        )
    ]
